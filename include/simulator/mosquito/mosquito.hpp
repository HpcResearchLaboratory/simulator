...
struct Mosquito {
  ...
  __host__ __device__ Mosquito();
};

struct EstaMortoMosquito {

  __host__ __device__ bool operator()(Mosquito mosquito);
};
...
struct LessQuadraMosquito {
  ...
  __host__ __device__ bool operator()(Mosquito mosquito1, Mosquito mosquito2);
};

struct ToQuadraMosquito {

  __host__ __device__ int operator()(Mosquito mosquito);
};
...
public:
  ...
  Ambiente *ambiente;
  Mosquito *mosquitos;
  int nMosquitos, maxMosquitos;
  DVector<Mosquito> *mosquitosDev;
  Mosquito *PmosquitosDev;
  DVector<int> *indMosquitosDev;
  int *PindMosquitosDev;
  int sizeIndMosquitos, nDistRurMos;
...
private:
  ...
  void inicializarMosquito(int id, int s, int sw, int fs, int ie, int sd,
                           int st, int q, int l, int x, int y);

  void inserirMosquitos(int quantidade, int s, int sw, int fs, int sd, int st,
                        int &i);

  void inserirOvosEmFocos(int quantidade, int s, int sw, int fs, int sd, int st,
                          int &i);
