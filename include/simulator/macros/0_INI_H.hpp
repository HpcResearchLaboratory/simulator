...
#define PROBABILIDADE_DISTRIBUICAO_HUMANOS(per)                                \
  (double)(ENTRE_FAIXA(parametros->parametros[DESL_0_INI_H + 384],             \
                       parametros->parametros[DESL_0_INI_H + 385], per))

#define PROBABILIDADE_HUMANO_ASSINTOMATICO                                     \
  (double)(ENTRE_FAIXA(parametros[DESL_0_INI_H + 386],                         \
                       parametros[DESL_0_INI_H + 387], randPerc))

#define PROBABILIDADE_HUMANO_ASSINTOMATICO_(per)                               \
  (double)(ENTRE_FAIXA(parametros->parametros[DESL_0_INI_H + 386],             \
                       parametros->parametros[DESL_0_INI_H + 387], per))

#define SOROTIPO_PREDOMINANTE (int)(parametros->parametros[DESL_0_INI_H + 388])

#define PROBABILIDADE_SOROTIPO_PREDOMINANTE(per)                               \
  (double)(ENTRE_FAIXA(parametros->parametros[DESL_0_INI_H + 390],             \
                       parametros->parametros[DESL_0_INI_H + 391], per))
