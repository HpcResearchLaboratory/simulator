...
*/
__host__ __device__ int PreInsercaoMosquitos::operator()(int id) {
  int nMosquitos = 0;
...
*/
InsercaoMosquitos::InsercaoMosquitos(Mosquitos *mosquitos, Ambiente *ambiente,
...
*/
__host__ __device__ void InsercaoMosquitos::operator()(int id) {
  dre &seed = seeds[id];
  ...
  for (int j = 0; j < sizeDistMosquitos; j++) {
    ...
    for (int k = 0; k < quant; k++) {
      ...
      if (sizePontEst > 0 and
          randPerc <= PROBABILIDADE_DISTRIBUICAO_MOSQUITOS) {
      ...
      switch (fs) {
      case OVO:
        ie = IDADE_MOSQUITOS_NAO_ALADOS(fs);
        break;
      case ATIVA:
        ie = IDADE_MOSQUITOS_ATIVOS(s, sw);
        break;
      case DECADENTE:
        ie = IDADE_MOSQUITOS_DECADENTES(s, sw);
        break;
      }
...
InsercaoMosquitos::inicializarMosquito(int id, int s, int sw, int fs, int ie,
