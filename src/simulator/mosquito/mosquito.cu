...
#include <simulator/macros/0_INI_M.hpp>
#include <simulator/macros/0_SIM.hpp>
...
*/
__host__ __device__ Mosquito::Mosquito() {
...
*/
__host__ __device__ bool EstaMortoMosquito::operator()(Mosquito mosquito) {
...
MosquitoFemeaSuscetivelAlado::operator()(Mosquito mosquito) {
...
*/
__host__ __device__ bool MosquitoFemeaAlado::operator()(Mosquito mosquito) {
  bool vivo = GET_VD_M_ == VIVO;
  bool femea = GET_S_M_ == FEMEA;
  bool ativo = GET_FS_M_ == ATIVA;
  bool decadente = GET_FS_M_ == DECADENTE;
...
*/
__host__ __device__ bool LessQuadraMosquito::operator()(Mosquito mosquito1,
                                                        Mosquito mosquito2) {
...
*/
__host__ __device__ int ToQuadraMosquito::operator()(Mosquito mosquito) {
  return GET_Q_M_;
}
...
void Mosquitos::atualizacaoIndices() {
  ...
  sort(mosquitosDev->begin(), mosquitosDev->end(), LessQuadraMosquito());

  // Conversão dos agentes mosquitos para identificadores de quadra.
  DVector<int> k1(nMosquitos);
  transform(mosquitosDev->begin(), mosquitosDev->end(), k1.begin(),
            ToQuadraMosquito());

  // São obtidas as quantidades de agentes por quadra.
  reduce_by_key(k1.begin(), k1.end(), v1, k2.begin(), v2.begin());
  ...
  if (nQuadrasSemAgentes > 0) {
    ...
    set_difference(t, t + ambiente->nQuadras, k2.begin(), k2.end(),
                   quadrasSemAgentes.begin());
  ...
  inclusive_scan(v2.begin(), v2.end(), indMosquitosDev->begin() + 1);
}
...
void Mosquitos::toGPU() {
  ...
  copy_n(ambiente->capFocos, ambiente->sizeFocos,
         ambiente->capFocosDev->begin());
}
...
*/
void Mosquitos::inicializarMosquito(int id, int s, int sw, int fs, int ie,
...
*/
void Mosquitos::inserirMosquitos(int quantidade, int s, int sw, int fs, int sd,
                                 int st, int &i) {
  int p, l, q, ie;
  RandPerc rand;
  ...
  for (int j = 0; j < quantidade; ++j) {
    ...
    int x =
        ambiente->pos[ambiente->indPos[ambiente->indQuadras[q * 2] + l] + p].x;
    int y =
        ambiente->pos[ambiente->indPos[ambiente->indQuadras[q * 2] + l] + p].y;

    // Escolhe a idade inicial do agente.
    switch (fs) {
    case ATIVA:
      ie = IDADE_MOSQUITOS_ATIVOS_(s, sw, rand());
      break;
    case DECADENTE:
      ie = IDADE_MOSQUITOS_DECADENTES_(s, sw, rand());
      break;
    }
...
/*
  ...
  Parâmetros:
    ...
    i           - Referência à última posição ocupada dentro do vetor
*/
void Mosquitos::inserirOvosEmFocos(int quantidade, int s, int sw, int fs,
                                   int sd, int st, int &i) {
  RandPerc rand;
  for (int j = 0; j < quantidade; j++) {
