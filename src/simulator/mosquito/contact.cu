...
#include <simulator/macros/2_CON_M.hpp>
...
*/
ContatoMosquitos::ContatoMosquitos(Mosquitos *mosquitos, Ambiente *ambiente,
                                   Parametros *parametros, int periodo,
                                   Seeds *seeds) {
...
*/
__host__ __device__ void ContatoMosquitos::operator()(int id) {
  dre &seed = seeds[id];
  ...
  for (int idFemea = indInicial; idFemea < indFinal; ++idFemea) {
    ...
    if (l_f != l or x_f != x or y_f != y)
      continue;

    // Ignora agentes mortos ou machos
    if (vd_f == MORTO or s_f != FEMEA)
      continue;

    // Ignora ovos, larvas e pupas
    if (fs_f != ATIVA and fs_f != DECADENTE)
      continue;
...
*/
__host__ __device__ int ContatoMosquitos::getMacho(int x, int y, int l, int q) {
  int id = -1;
  int nAcasal = INT_MAX;
