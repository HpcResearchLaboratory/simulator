...
#include <simulator/macros/1_MOV_M.hpp>
...
*/
__host__ __device__ void MovimentacaoMosquitos::operator()(int id) {
  dre &seed = seeds[id];
  ...
  if (vd == MORTO or (fs != ATIVA and fs != DECADENTE))
    return;

  // Movimentação no periodo da manhã e da tarde
  if ((periodo == MANHA or periodo == TARDE) and
      randPerc <= TAXA_MOBILIDADE_DIURNA_MOSQUITOS(s)) {
...
MovimentacaoMosquitos::movimentacaoDiurna(int id, dre &seed,
                                          urd<double> &dist) {
  if (GET_S_M(id) == MACHO) {
...
*/
__host__ __device__ void
MovimentacaoMosquitos::movimentacaoAleatoria(int id, dre &seed,
                                             urd<double> &dist) {
...
*/
__host__ __device__ void
MovimentacaoMosquitos::movimentacaoLocal(int id, dre &seed, urd<double> &dist) {
  int q = GET_Q_M(id), l = GET_L_M(id), x = GET_X_M(id), y = GET_Y_M(id);
  int k, n;
...
*/
__host__ __device__ void
MovimentacaoMosquitos::movimentacaoFemea(int id, dre &seed, urd<double> &dist) {
...
*/
__host__ __device__ void
MovimentacaoMosquitos::movimentacaoFemeaNaoAcasalada(int id, dre &seed,
                                                     urd<double> &dist) {
...
*/
__host__ __device__ void
MovimentacaoMosquitos::movimentacaoFemeaAcasalada(int id, dre &seed,
                                                  urd<double> &dist) {
  if (not GET_AM_M(id)) {
...
*/
__host__ __device__ void MovimentacaoMosquitos::movimentacaoAleatoriaVooLevy(
...
*/
__host__ __device__ bool MovimentacaoMosquitos::buscaMacho(int id, dre &seed,
                                                           urd<double> &dist) {
  ...
  if (not temMacho(x, y, l, q, tu, raioBuscaMacho))
    return false;
...
*/
__host__ __device__ bool MovimentacaoMosquitos::buscaPE(int id, dre &seed,
                                                        urd<double> &dist) {
  int x = GET_X_M(id), y = GET_Y_M(id), l = GET_L_M(id), q = GET_Q_M(id);
  int x_pe, y_pe, k;
  ...
  if (not temPE(x, y, l, q, tu, raioBuscaPE))
    return false;
...
*/
__host__ __device__ bool MovimentacaoMosquitos::buscaHumano(int id, dre &seed,
                                                            urd<double> &dist) {
  int x = GET_X_M(id), y = GET_Y_M(id), l = GET_L_M(id), q = GET_Q_M(id);
  int x_h, y_h, k;
  ...
  if (not(temHumano(x, y, l, q, tu, raioBuscaHumano)))
    return false;
...
*/
__host__ __device__ bool MovimentacaoMosquitos::buscaFoco(int id, dre &seed,
                                                          urd<double> &dist) {
  ...
  for (int i = inicioFocos; i < finalFocos; i++) {
    ...
    if (x == x_foco and y == y_foco) {
      if (nAgentes < 0.6 * LIMITE_FOCOS) {
        ...
        if (nAgentes < LIMITE_FOCOS)
          nAgentesFocoAtual = nAgentes;
        continue;
      }
    }
  ...
  if (focoPreferencial == -1)
    return false;

  // Fica no foco atual caso seja mais vantajoso
  if (capFocos[focoPreferencial] >= nAgentesFocoAtual)
    return true;
  // Caso o foco preferencial já esteja lotado retorna falso
  if (capFocos[focoPreferencial] >= LIMITE_FOCOS)
    return false;
...
/*
  ...
  na tupla "tu".
*/
__host__ __device__ bool MovimentacaoMosquitos::temMacho(int x, int y, int l,
                                                         int q,
                                                         tuple<int, int> &tu,
                                                         double raio) {
  int ind = 0;
...
/*
  ...
  "tu".
*/
__host__ __device__ bool MovimentacaoMosquitos::temPE(int x, int y, int l,
                                                      int q,
                                                      tuple<int, int> &tu,
                                                      double raio) {
...
/*
  ...
  na tupla "tu".
*/
__host__ __device__ bool MovimentacaoMosquitos::temHumano(int x, int y, int l,
                                                          int q,
                                                          tuple<int, int> &tu,
                                                          double raio) {
  int ind = 0;
...
*/
__host__ __device__ bool MovimentacaoMosquitos::estaEmPE(int quadra, int lote) {
  for (int i = 0; i < sizePontEst; i += 2) {
...
*/
__host__ __device__ void MovimentacaoMosquitos::vooLevy(int id, dre &seed,
                                                        urd<double> &dist) {
...
*/
__host__ __device__ void
MovimentacaoMosquitos::moveMosquitoParaRua(int idMosquito) {
...
*/
__host__ __device__ int MovimentacaoMosquitos::nVertVizinhos(int x, int y,
                                                             int l, int q) {
  int nVert = 0;
...
MovimentacaoMosquitos::nVertVizinhos(int x, int y, int l, int q, int qd) {
  int nVert = 0;
  int inicio = indViz[indQuadras[2 * q] + l];
  int fim = indViz[indQuadras[2 * q] + l + 1];
  for (int i = inicio; i < fim; i++) {
...
*/
__host__ __device__ int MovimentacaoMosquitos::getVertK(int k, int x, int y,
                                                        int l, int q) {
  int j = 0;
  int inicio = indViz[indQuadras[2 * q] + l];
  int fim = indViz[indQuadras[2 * q] + l + 1];
...
*/
__host__ __device__ int
MovimentacaoMosquitos::getVertVL(int x, int y, int l, int q, int xo, int yo,
  ...
  if (nVert == 0)
    return -1; // Caso não encontre nenhuma posição
...
/*
  ...
  destino (xd, yd).
*/
__host__ __device__ int MovimentacaoMosquitos::getVertProxVizIn(int x, int y,
                                                                int l, int q,
                                                                int xd,
                                                                int yd) {
  int ind = 0;
...
*/
__host__ __device__ int
MovimentacaoMosquitos::getVertProxVizIn(int x, int y, int l, int q, int xd,
                                        int yd, int ld, int qd) {
  int ind = 0;
  double menorDist = INT_MAX;
  int inicio = indViz[indQuadras[2 * q] + l];
  int fim = indViz[indQuadras[2 * q] + l + 1];
...
*/
__host__ __device__ void MovimentacaoMosquitos::moveMosquito(int id, int k) {
  if (k != -1) {
