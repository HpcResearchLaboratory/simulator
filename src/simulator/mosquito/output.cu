...
/*
  ...
  para os agentes mosquitos, visão Dengue. A variável "popT" armazena os
...
*/
__host__ __device__ void ContPopTMD::operator()(int id) {
...
*/
__host__ __device__ void ContPopQMD::operator()(int id) {
  for (int i = 0; i < nMosquitos; ++i) {
    // Agentes mortos não são contabilizados.
    if (GET_VD_M(i) == MORTO)
      continue;
    ...
    // Somente contabiliza o agente em uma coluna da saída.
    if (desl == id)
      popQ[indPopQ[q] + VEC(ciclo, desl, N_COLS_MD)]++;
  }
}
...
*/
ContEspacialMD::ContEspacialMD(Mosquitos *mosquitos, Saidas *saidas,
...
*/
__host__ __device__ void ContEspacialMD::operator()(int id) {
  ...
  for (int i = indMosquitos[q]; i < indMosquitos[q + 1]; ++i) {
    ...
    if (GET_VD_M(i) == MORTO or GET_L_M(i) != l or GET_X_M(i) != x or
        GET_Y_M(i) != y)
      continue;
...
*/
__host__ __device__ void ContPopTMW::operator()(int id) {
  for (int i = 0; i < nMosquitos; ++i) {
    // Agentes mortos não são contabilizados.
    if (GET_VD_M(i) == MORTO)
      continue;
...
*/
__host__ __device__ void ContPopQMW::operator()(int id) {
  for (int i = 0; i < nMosquitos; ++i) {
    // Agentes mortos não são contabilizados.
    if (GET_VD_M(i) == MORTO)
      continue;
    ...
    // Somente contabiliza o agente em uma coluna da saída.
    if (desl == id)
      popQ[indPopQ[q] + VEC(ciclo, desl, N_COLS_MW)]++;
  }
}
...
/*
  ...
  acumuladas totais para os agentes mosquitos, visão Dengue. A variável
  "popNovoT" armazena os resultados gerados pelo método "operator()". Esta
  classe é responsável pela geração dos resultados armazenados no arquivo
  "Saidas/MonteCarlo_{1}/Quantidades_Mosquitos_Dengue_Novo_Total.csv".
*/
ContPopNovoTMD::ContPopNovoTMD(Mosquitos *mosquitos, Saidas *saidas,
                               int ciclo) {
  this->mosquitos = mosquitos->PmosquitosDev;
  this->nMosquitos = mosquitos->nMosquitos;
...
*/
__host__ __device__ void ContPopNovoTMD::operator()(int id) {
  for (int i = 0; i < nMosquitos; ++i) {
    // Agentes mortos não são contabilizados.
    if (GET_VD_M(i) == MORTO)
      continue;
    ...
    // Somente contabiliza o agente em uma coluna da saída.
    if (desl == id)
      popNovoT[VEC(ciclo, desl, N_COLS_MD)]++;
  }
}
...
*/
ContPopNovoQMD::ContPopNovoQMD(Mosquitos *mosquitos, Saidas *saidas,
                               int ciclo) {
  this->mosquitos = mosquitos->PmosquitosDev;
  this->nMosquitos = mosquitos->nMosquitos;
  this->indPopQ = saidas->PindPopQMDDev;
...
*/
__host__ __device__ void ContPopNovoQMD::operator()(int id) {
  for (int i = 0; i < nMosquitos; ++i) {
    // Agentes mortos não são contabilizados.
    if (GET_VD_M(i) == MORTO)
      continue;
