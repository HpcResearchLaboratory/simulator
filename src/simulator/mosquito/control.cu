#include <simulator/mosquito/control.hpp>
#include <simulator/environment.hpp>
#include <simulator/macros/3_TRA_M.hpp>
#include <simulator/macros/4_CON_M.hpp>
#include <simulator/macros/general.hpp>
#include <simulator/macros/mosquito.hpp>
...
*/
__host__ __device__ void ControleNaturalMosquitosPorIdade::operator()(int id) {
  int idMosquito = id;
  dre &seed = seeds[id];
  urd<double> dist(0.0, 1.0);

  // Filtra apenas mosquitos vivos e na fase decadente
  if (GET_VD_M(idMosquito) == MORTO)
    return;
  if (GET_FS_M(idMosquito) != DECADENTE)
    return;
...
ControleNaturalMosquitosPorSelecao::ControleNaturalMosquitosPorSelecao(
    Mosquitos *mosquitos, Ambiente *ambiente, Parametros *parametros, int ciclo,
    int idLira, Seeds *seeds) {
  this->mosquitos = mosquitos->PmosquitosDev;
...
ControleNaturalMosquitosPorSelecao::operator()(int id) {
  dre &seed = seeds[id];
  urd<double> dist(0.0, 1.0);

  if (not(FREQ_CONTROLE_NATURAL_SELECAO > 0 and ciclo > 0 and
          ciclo % FREQ_CONTROLE_NATURAL_SELECAO == 0))
    return;
  ...
  if (idLira != -1) {
    tMSO *= ENTRE_FAIXA(cli[ciclo].txMinNaoAlados, cli[ciclo].txMaxNaoAlados,
                        randPerc);
    tMWO *= ENTRE_FAIXA(cli[ciclo].txMinNaoAlados, cli[ciclo].txMaxNaoAlados,
                        randPerc);
    tFSO *= ENTRE_FAIXA(cli[ciclo].txMinNaoAlados, cli[ciclo].txMaxNaoAlados,
                        randPerc);
    tFWO *= ENTRE_FAIXA(cli[ciclo].txMinNaoAlados, cli[ciclo].txMaxNaoAlados,
                        randPerc);
    tMSL *= ENTRE_FAIXA(cli[ciclo].txMinNaoAlados, cli[ciclo].txMaxNaoAlados,
                        randPerc);
    tMWL *= ENTRE_FAIXA(cli[ciclo].txMinNaoAlados, cli[ciclo].txMaxNaoAlados,
                        randPerc);
    tFSL *= ENTRE_FAIXA(cli[ciclo].txMinNaoAlados, cli[ciclo].txMaxNaoAlados,
                        randPerc);
    tFWL *= ENTRE_FAIXA(cli[ciclo].txMinNaoAlados, cli[ciclo].txMaxNaoAlados,
                        randPerc);
    tMSP *= ENTRE_FAIXA(cli[ciclo].txMinNaoAlados, cli[ciclo].txMaxNaoAlados,
                        randPerc);
    tMWP *= ENTRE_FAIXA(cli[ciclo].txMinNaoAlados, cli[ciclo].txMaxNaoAlados,
                        randPerc);
    tFSP *= ENTRE_FAIXA(cli[ciclo].txMinNaoAlados, cli[ciclo].txMaxNaoAlados,
                        randPerc);
    tFWP *= ENTRE_FAIXA(cli[ciclo].txMinNaoAlados, cli[ciclo].txMaxNaoAlados,
                        randPerc);
    tMSA *=
        ENTRE_FAIXA(cli[ciclo].txMinAlados, cli[ciclo].txMaxAlados, randPerc);
    tMWA *=
        ENTRE_FAIXA(cli[ciclo].txMinAlados, cli[ciclo].txMaxAlados, randPerc);
    tFSA *=
        ENTRE_FAIXA(cli[ciclo].txMinAlados, cli[ciclo].txMaxAlados, randPerc);
...
*/
__host__ __device__ void ControleNaturalMosquitosPorSelecao::remove(
    dre &seed, urd<double> &dist, int idM, int s, int sw, int fs, double prob) {
...
*/
__host__ __device__ int ControleNaturalMosquitosPorSelecao::getIdFoco(int idM) {
  int x = GET_X_M(idM);
  int y = GET_Y_M(idM);
  int q = GET_Q_M(idM);
  int l = GET_L_M(idM);
...
ControleNaturalMosquitosPorSelecao::estaEmFoco(int x, int y, int q, int l) {
...
*/
__host__ __device__ bool ControleNaturalMosquitosPorSelecao::estaEmPE(int q,
                                                                      int l) {
...
*/
PreControlesMosquitos::PreControlesMosquitos(Ambiente *ambiente,
...
*/
__host__ __device__ void PreControlesMosquitos::operator()(int id) {
  dre &seed = seeds[id];
  urd<double> dist(0.0, 1.0);

  if ((contr[idContr].tipoControle == 'R' or
       contr[idContr].tipoControle == 'T' or
       contr[idContr].tipoControle == 'P') and
      ciclo == contr[idContr].ciclo) {
    contr[idContr].efeRes = ENTRE_FAIXA(
        contr[idContr].taxaMinQuimico, contr[idContr].taxaMaxQuimico, randPerc);
  }
}
...
*/
ControlesMosquitos::ControlesMosquitos(Mosquitos *mosquitos, Ambiente *ambiente,
                                       Parametros *parametros, int ciclo,
                                       int idContr, int sizePontEst,
                                       Seeds *seeds) {
  this->mosquitos = mosquitos->PmosquitosDev;
  this->indMosquitos = mosquitos->PindMosquitosDev;
  this->parametros = parametros->PparametrosDev;
...
*/
__host__ __device__ void ControlesMosquitos::operator()(int id) {
  dre &seed = seeds[id];
  urd<double> dist(0.0, 1.0);
...
*/
__host__ __device__ bool ControlesMosquitos::temRaio(int q, int l, int x,
                                                     int y) {
  ...
  for (int i = indRaios[idContr]; i < indRaios[idContr + 1]; ++i) {
    if (raios[i].quadra == q and raios[i].lote == l and raios[i].x == x and
        raios[i].y == y) {
      return true;
    }
  }
...
/*
  ...
  em determinada posição do ambiente.
*/
__host__ __device__ bool ControlesMosquitos::temBloqueio(int q, int l, int x,
                                                         int y) {
...
/*
  ...
  em determinada posição do ambiente.
*/
__host__ __device__ bool ControlesMosquitos::temPE(int q, int l) {
...
/*
  ...
  em determinada posição do ambiente.
*/
__host__ __device__ bool ControlesMosquitos::temTratamento(int q, int l, int x,
                                                           int y) {
...
*/
__host__ __device__ void ControlesMosquitos::controleQuimicoNaoAlados(
    dre &seed, urd<double> &dist, int x, int y, int l, int q, double taxa) {
  int l_m, x_m, y_m, vd_m, fs_m;

  for (int idMosquito = indMosquitos[q]; idMosquito < indMosquitos[q + 1];
       ++idMosquito) {
    l_m = GET_L_M(idMosquito);
    x_m = GET_X_M(idMosquito);
    y_m = GET_Y_M(idMosquito);
    vd_m = GET_VD_M(idMosquito);
    fs_m = GET_FS_M(idMosquito);

    if (vd_m == MORTO or l_m != l or x_m != x or y_m != y or fs_m == ATIVA or
        fs_m == DECADENTE)
      continue;
...
*/
__host__ __device__ void
ControlesMosquitos::controleMecanicoNaoAlados(dre &seed, urd<double> &dist,
...
*/
__host__ __device__ void
ControlesMosquitos::controleQuimicoAlados(dre &seed, urd<double> &dist, int x,
                                          int y, int l, int q) {
...
*/
PosControlesMosquitos::PosControlesMosquitos(Ambiente *ambiente,
...
*/
__host__ __device__ void PosControlesMosquitos::operator()(int id) {
  dre &seed = seeds[id];
  urd<double> dist(0.0, 1.0);

  if (contr[idContr].efeRes > 0 and (contr[idContr].tipoControle == 'R' or
                                     contr[idContr].tipoControle == 'T' or
                                     contr[idContr].tipoControle == 'P')) {
...
*/
PreControleBiologico::PreControleBiologico(Parametros *parametros, int ciclo) {
  this->parametros = parametros->PparametrosDev;
  this->ciclo = ciclo;
}
...
*/
__host__ __device__ int PreControleBiologico::operator()(int id) {
  int nMosquitos = 0;
...
*/
ControleBiologico::ControleBiologico(Mosquitos *mosquitos, Ambiente *ambiente,
                                     Parametros *parametros, int ciclo,
                                     bool alocarMosquitos, Seeds *seeds) {
  this->mosquitos = mosquitos->PmosquitosDev;
  this->parametros = parametros->PparametrosDev;
  this->ciclo = ciclo;
...
*/
__host__ __device__ void ControleBiologico::operator()(int id) {
  dre &seed = seeds[id];
  urd<double> dist(0.0, 1.0);

  if (not(alocarMosquitos and FREQ_CONTROLE_BIOLOGICO > 0 and ciclo > 0 and
          ciclo % FREQ_CONTROLE_BIOLOGICO == 0))
    return;
  ...
  inserirMosquitos(seed, dist, nMachos, MACHO, WOLBACHIA, ATIVA, SUSCETIVEL, 0,
                   quadConBio, i);
  inserirMosquitos(seed, dist, nFemeas, FEMEA, WOLBACHIA, ATIVA, SUSCETIVEL, 0,
                   quadConBio, i);
}
...
*/
__host__ __device__ void
ControleBiologico::inicializarMosquito(int id, int s, int sw, int fs, int ie,
                                       int sd, int st, int q, int l, int x,
                                       int y) {
  SET_S_M(id, s);
  SET_SW_M(id, sw);
  SET_FS_M(id, fs);
...
*/
__host__ __device__ void
ControleBiologico::inserirMosquitos(dre &seed, urd<double> &dist,
                                    int quantidade, int s, int sw, int fs,
                                    int sd, int st, int q, int &i) {
  int p, x, y, l, ie;

  for (int j = 0; j < quantidade; ++j) {
    l = ENTRE_FAIXA(0, nLotes[q], randPerc);

    p = (indPos[indQuadras[2 * q] + l + 1] - indPos[indQuadras[2 * q] + l]);
...
*/
TratamentoAmbiental::TratamentoAmbiental(Mosquitos *mosquitos,
                                         Ambiente *ambiente,
                                         Parametros *parametros, int ciclo,
                                         int sizeConAmb, Seeds *seeds) {
  this->mosquitos = mosquitos->PmosquitosDev;
  this->indMosquitos = mosquitos->PindMosquitosDev;
  this->parametros = parametros->PparametrosDev;
  this->ciclo = ciclo;
...
*/
__host__ __device__ void TratamentoAmbiental::operator()(int id) {
  dre &seed = seeds[id];
  urd<double> dist(0.0, 1.0);

  if (not(FREQ_TRATAMENTO_AMBIENTAL > 0 and ciclo > 0 and
          ciclo % FREQ_TRATAMENTO_AMBIENTAL == 0))
    return;
  ...
  for (int idM = indMosquitos[quadConAmb]; idM < indMosquitos[quadConAmb + 1];
       ++idM) {
