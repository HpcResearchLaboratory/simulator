...
/*
  ...
  contabilizados.
*/
PreGeracao::PreGeracao(Ambiente *ambiente, Mosquitos *mosquitos,
...
*/
__host__ __device__ int PreGeracao::operator()(int id) {
  dre &seed = seeds[id];
  ...
  if (vd_m == MORTO or s_m == MACHO)
    return 0;
  if (fs_m != ATIVA and fs_m != DECADENTE)
    return 0;
  if (ta_m == NENHUM)
    return 0;
...
*/
__host__ __device__ bool PreGeracao::estaEmFoco(int id) {
  int x = GET_X_M(id);
  int y = GET_Y_M(id);
  int q = GET_Q_M(id);
  int l = GET_L_M(id);
...
*/
Geracao::Geracao(Ambiente *ambiente, Mosquitos *mosquitos,
                 Parametros *parametros, Seeds *seeds) {
  this->mosquitos = mosquitos->PmosquitosDev;
...
*/
__host__ __device__ void Geracao::operator()(int id) {
  // dre& seed = seeds[id];
  urd<double> dist(0.0, 1.0);
  int i = 0;
  ...
  for (int id = 0; id < nMosquitos; ++id) {
    ...
    if (pr_m == ESTERIL or not estaEmFoco(id))
      continue;
...
*/
__host__ __device__ void Geracao::postura(int id, int &i) {
  dre &seed = seeds[id];
  urd<double> dist(0.0, 1.0);
...
*/
__host__ __device__ void Geracao::inserirOvos(int id, int &i, int total,
                                              double frac, int sw) {
  dre &seed = seeds[id];
  urd<double> dist(0.0, 1.0);
...
*/
__host__ __device__ void Geracao::inserirMosquitos(int quantidade, int s,
                                                   int sw, int fs, int sd,
                                                   int st, int q, int l, int x,
                                                   int y, int &i) {
...
*/
__host__ __device__ void Geracao::inicializarMosquito(int id, int s, int sw,
                                                      int fs, int sd, int st,
                                                      int q, int l, int x,
                                                      int y) {
  SET_S_M(id, s);
  SET_SW_M(id, sw);
...
/*
  Verifica se a posição atual de determinado mosquito é um ponto de foco.
*/
__host__ __device__ bool Geracao::estaEmFoco(int id) {
  int x = GET_X_M(id);
  int y = GET_Y_M(id);
  int q = GET_Q_M(id);
  int l = GET_L_M(id);
...
*/
__host__ __device__ int Geracao::getIdFoco(int id) {
  int x = GET_X_M(id);
  int y = GET_Y_M(id);
  int q = GET_Q_M(id);
  int l = GET_L_M(id);
