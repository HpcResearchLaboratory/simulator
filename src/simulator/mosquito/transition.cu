...
*/
TransicaoFasesMosquitos::TransicaoFasesMosquitos(Ambiente *ambiente,
                                                 Mosquitos *mosquitos,
...
*/
__host__ __device__ void TransicaoFasesMosquitos::operator()(int id) {
  dre &seed = seeds[id];
  ...
  for (int i = indMosquitos[q]; i < indMosquitos[q + 1]; i++) {
    ...
    if (GET_Q_M(idMosquito) != q)
      continue;
    if (GET_L_M(idMosquito) != l)
      continue;
    if (GET_X_M(idMosquito) != x)
      continue;
    if (GET_Y_M(idMosquito) != y)
      continue;
    ...
    if (vd == MORTO or fs >= DECADENTE)
      continue;

    // Ignora agentes que ainda não atingiram a idade necessária
    if (fs == ATIVA) {
      if (ie < IDADE_MAX_MOSQUITOS_ATIVOS(s, sw))
        continue;
    } else {
      if (ie < IDADE_MAX_MOSQUITOS_NAO_ALADOS(fs))
        continue;
    }
...
*/
__host__ __device__ int TransicaoFasesMosquitos::getIdFoco(int x, int y, int q,
                                                           int l) {
...
*/
TransicaoEstadosMosquitos::TransicaoEstadosMosquitos(Mosquitos *mosquitos,
                                                     Parametros *parametros,
                                                     Seeds *seeds) {
  this->mosquitos = mosquitos->PmosquitosDev;
  this->parametros = parametros->PparametrosDev;
...
*/
__host__ __device__ void TransicaoEstadosMosquitos::operator()(int id) {
  int idMosquito = id;
  dre &seed = seeds[id];
  urd<double> dist(0.0, 1.0);
