...
/*
  ...
  A variável "ambiente" armazena o ambiente de simulação. A variável
  "parametros" armazena os parâmetros de simulação e a variável "saidaMC"
  armazena o caminho para a pasta de saída dos arquivos resultantes da simulação
  tipo Monte Carlo.
...
Saidas::~Saidas() {
  ...
  delete (popQMWDev);
  delete (espacialHDev);
  delete (espacialMDDev);
  delete (espacialNovoHDev);
  delete (popNovoTHDev);
  delete (popNovoTMDDev);
  delete (popNovoQHDev);
...
void Saidas::toGPU() {
  ...
  espacialNovoHDev =
      new DVector<int>(espacialNovoH, espacialNovoH + sizeEspacialNovoH);
