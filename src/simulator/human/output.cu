...
*/
__host__ __device__ void ContPopTH::operator()(int id) {
...
*/
__host__ __device__ void ContPopQH::operator()(int id) {
  for (int i = 0; i < nHumanos; ++i) {
    // Agentes mortos não são contabilizados.
    if (GET_SD_H(i) == MORTO)
      continue;
    ...
    // Somente contabiliza o agente em uma coluna da saída.
    if (desl == id)
      popQ[indPopQ[q] + VEC(ciclo, desl, N_COLS_H)]++;
  }
}
...
*/
__host__ __device__ void ContEspacialH::operator()(int id) {
  ...
  for (int i = indHumanos[q]; i < indHumanos[q + 1]; ++i) {
    ...
    if (GET_SD_H(i) == MORTO or GET_L_H(i) != l or GET_X_H(i) != x or
        GET_Y_H(i) != y)
      continue;
...
*/
__host__ __device__ void ContPopNovoTH::operator()(int id) {
  for (int i = 0; i < nHumanos; ++i) {
    // Agentes mortos não são contabilizados.
    if (GET_SD_H(i) == MORTO)
      continue;
    ...
    // Somente contabiliza o agente em uma coluna da saída.
    if (desl == id)
      popNovoT[VEC(ciclo, desl, N_COLS_H)]++;
  }
}
...
*/
__host__ __device__ void ContPopNovoQH::operator()(int id) {
  for (int i = 0; i < nHumanos; ++i) {
    // Agentes mortos não são contabilizados.
    if (GET_SD_H(i) == MORTO)
      continue;
...
*/
__host__ __device__ void ContEspacialNovoH::operator()(int id) {
  int x = pos[id].x, y = pos[id].y;
  int l = pos[id].lote, q = pos[id].quadra;
  int d = VEC(id, ciclo, nCiclos), e;
