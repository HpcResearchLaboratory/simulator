...
*/
__host__ __device__ void TransicaoEstadosHumanos::operator()(int id) {
  int idHumano = id;
  dre &seed = seeds[id];
...
TransicaoEstadosHumanos::transicaoExposto(int idHumano, urd<double> dist,
                                          dre &seed) {
...
*/
__host__ __device__ void
TransicaoEstadosHumanos::transicaoInfectante(int idHumano, urd<double> dist,
                                             dre &seed) {
...
*/
__host__ __device__ void
TransicaoEstadosHumanos::transicaoHemorragico(int idHumano, urd<double> dist,
                                              dre &seed) {
  int c = GET_C_H(idHumano), fe = GET_FE_H(idHumano);
...
*/
__host__ __device__ void
TransicaoEstadosHumanos::transicaoRecuperado(int idHumano, urd<double> dist,
                                             dre &seed) {
  int c = GET_C_H(idHumano), fe = GET_FE_H(idHumano);
...
*/
__host__ __device__ int TransicaoEstadosHumanos::contarSorotipos(int sc) {
...
/*
  ...
  agentes afetados.
*/
CampanhaVacinacao::CampanhaVacinacao(Humanos *humanos, Ambiente *ambiente,
                                     Parametros *parametros, int ciclo,
                                     int sizeQuadVac, int sizeFEVac,
...
*/
__host__ __device__ void CampanhaVacinacao::operator()(int id) {
  if (not periodoCampanhaVacinacao())
    return;
  ...
  for (int idHumano = indHumanos[l]; idHumano < indHumanos[l + 1]; ++idHumano) {
    ...
    if (not faixaEtariaTeraVacinacao(fe_h))
      continue;
...
*/
__host__ __device__ bool CampanhaVacinacao::periodoCampanhaVacinacao() {
...
*/
__host__ __device__ bool CampanhaVacinacao::faixaEtariaTeraVacinacao(int fe) {
...
*/
PosCampanhaVacinacao::PosCampanhaVacinacao(Ambiente *ambiente, int ciclo,
                                           int sizePerVac, int sizeCicVac) {
  this->ciclo = ciclo;
  this->sizePerVac = sizePerVac;
  this->sizeCicVac = sizeCicVac;
...
*/
__host__ __device__ void PosCampanhaVacinacao::operator()(int id) {
...
*/
InsercaoVacinados::InsercaoVacinados(Humanos *humanos, Ambiente *ambiente,
                                     Parametros *parametros, int ciclo,
                                     Seeds *seeds) {
  this->humanos = humanos->PhumanosDev;
  this->indHumanos = humanos->PindHumanosDev;
...
*/
__host__ __device__ void InsercaoVacinados::operator()(int id) {
  // Filtra as inserções passadas e futuras
  if (vacs[id].processado)
    return;
  if (vacs[id].ciclo > ciclo)
    return;
  ...
  for (int idHumano = indHumanos[q]; idHumano < indHumanos[q + 1]; idHumano++) {
    ...
    if (q_h != q or s_h != s or fe_h != fe)
      continue;

    // Filtra apenas humanos suscetíveis / recuperados
    if (sd_h != SUSCETIVEL and sd_h != RECUPERADO)
      continue;
