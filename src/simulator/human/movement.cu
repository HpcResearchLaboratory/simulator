...
#include <simulator/macros/1_MOV_H.hpp>
...
*/
__host__ __device__ void MovimentacaoHumanos::operator()(int id) {
  dre &seed = seeds[id];
...
MovimentacaoHumanos::movimentacaoLocal(int id, dre &seed, urd<double> &dist) {
  // Acesso aos atributos do agente
...
*/
__host__ __device__ void
MovimentacaoHumanos::movimentacaoAleatoria(int id, dre &seed,
                                           urd<double> &dist) {
  // Acesso aos atributos do agente
  int l = GET_Q_H(id), q = GET_L_H(id);
  int x = GET_X_H(id), y = GET_Y_H(id);
...
*/
__host__ __device__ void
MovimentacaoHumanos::movimentacaoLivre(int id, dre &seed, urd<double> &dist) {
    ...
    if (q == qo and l == lo) {
      ...
      if (m) {
        ...
        if (k) {
          ...
          // k = getVertProxVizIn(x, y, q, l, fron[k].xDestino,
          // fron[k].yDestino, q, l);
          k = getVertProxVizEx(x, y, q, l, fron[k].xDestino, fron[k].yDestino,
                               q, l);
          // Move o agente
          moveHumano(id, k);
        }
...
*/
__host__ __device__ void
MovimentacaoHumanos::movimentacaoTrajeto(int id, dre &seed, urd<double> &dist) {
    ...
    if (l == lo and q == qo) {
      ...
      if (l == RUA) {
        ...
        for (int i = indFron[indQuadras[2 * ld] + qd];
             i < indFron[indQuadras[2 * ld] + qd + 1]; i++) {
        ...
        bool isUltimaRuaRota =
            (m + 1 ==
             ((indRotas[indTraj[t] + r + 1] - indRotas[indTraj[t] + r]) - 4));
...
*/
__host__ __device__ int MovimentacaoHumanos::nVertVizinhos(int x, int y, int q,
                                                           int l) {
  int n = 0;
...
MovimentacaoHumanos::nVertVizinhos(int x, int y, int q, int l, int qd, int ld) {
  int n = 0;
  int inicio = indViz[indQuadras[2 * l] + q];
  int fim = indViz[indQuadras[2 * l] + q + 1];
  for (int i = inicio; i < fim; i++) {
...
*/
__host__ __device__ int MovimentacaoHumanos::getVertK(int k, int x, int y,
                                                      int q, int l) {
  int j = 0;
  int inicio = indViz[indQuadras[2 * l] + q];
  int fim = indViz[indQuadras[2 * l] + q + 1];
...
*/
__host__ __device__ int MovimentacaoHumanos::getVertProxVizIn(int x, int y,
                                                              int q, int l,
                                                              int xd, int yd,
                                                              int ld) {
  int ind = -1;
...
/*
  Retorna a posição na vizinhança do ponto especificado (x, y) que está mais
  próxima de um ponto de destino desejado (xd, yd), com a restrição de que esta
  posição pertença à quadra "qd" e à localidade "ld". Exclui desta busca um
  ponto específico de minimo local (xmin, ymin).
*/
__host__ __device__ int
MovimentacaoHumanos::getVertProxVizIn(int x, int y, int q, int l, int xd,
                                      int yd, int qd, int ld, int xmin,
                                      int ymin) {
  int ind = -1;
  double menorDist = INT_MAX, dist;
  int inicio = indViz[indQuadras[2 * l] + q];
  ...
  for (int i = inicio; i < fim; i++) {
    if (viz[i].xOrigem == x and viz[i].yOrigem == y) {
      if (viz[i].loteDestino == qd and viz[i].quadraDestino == ld) {
        if (viz[i].xDestino == xmin and viz[i].yDestino == ymin)
          continue;
        dist = DIST(viz[i].xDestino, viz[i].yDestino, xd, yd);
        if (dist < menorDist) {
          menorDist = dist;
...
/*
  Retorna a posição na vizinhança do ponto especificado (x, y) que está mais
  próxima de um ponto de destino desejado (xd, yd), com a restrição de que esta
  posição pertença à localidade "ld". Adicionalmente, este método considera
  apenas posições mais próximas do ponto especificado do que o ponto de destino.
*/
__host__ __device__ int MovimentacaoHumanos::getVertProxVizEx(int x, int y,
                                                              int q, int l,
                                                              int xd, int yd,
                                                              int ld) {
  int ind = -1;
...
/*
  Retorna a posição na vizinhança do ponto especificado (x, y) que está mais
  próxima de um ponto de destino desejado (xd, yd), com a restrição de que esta
  posição pertença à quadra "qd" e à localidade "ld". Adicionalmente, este
  método considera apenas posições mais próximas do ponto especificado do que o
  ponto de destino.
*/
__host__ __device__ int MovimentacaoHumanos::getVertProxVizEx(int x, int y,
                                                              int q, int l,
...
/*
  ...
  próxima de um ponto de destino desejado (xd, yd), com a restrição de que esta
  posição pertença à quadra "qd" e à localidade "ld". Exclui desta busca um
  ponto específico de minimo local (xmin, ymin). Adicionalmente, este método
  considera apenas posições mais próximas do ponto especificado do que o ponto
  de destino.
*/
__host__ __device__ int
MovimentacaoHumanos::getVertProxVizEx(int x, int y, int q, int l, int xd,
                                      int yd, int qd, int ld, int xmin,
                                      int ymin) {
  int ind = -1;
  double menorDist = DIST(x, y, xd, yd), dist;
...
*/
__host__ __device__ int MovimentacaoHumanos::getVertProxEsqIn(int q, int xd,
                                                              int yd) {
  int ind = -1;
  double menorDist = INT_MAX, dist;
...
*/
__host__ __device__ void MovimentacaoHumanos::moveHumano(int id, int k) {
  if (k >= 0) {
