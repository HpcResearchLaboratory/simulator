...
#include <simulator/macros/0_INI_H.hpp>
...
*/
__host__ __device__ Humano::Humano() {
...
*/
__host__ __device__ bool EstaMortoHumano::operator()(Humano humano) {
...
*/
__host__ __device__ bool HumanoSuscetivel::operator()(Humano humano) {
...
*/
__host__ __device__ bool MenorIdLocalidadeHumano::operator()(Humano humano1,
                                                             Humano humano2) {
...
*/
__host__ __device__ int GetIdLocalidadeHumano::operator()(Humano humano) {
  return GET_Q_H_;
}
...
void Humanos::atualizacaoIndices() {
  ...
  sort(humanosDev->begin(), humanosDev->end(), MenorIdLocalidadeHumano());

  // Obtenção dos identificadores das localidades de cada humano.
  DVector<int> k1(nHumanos);
  transform(humanosDev->begin(), humanosDev->end(), k1.begin(),
            GetIdLocalidadeHumano());
  ...
  inclusive_scan(v2.begin(), v2.end(), indHumanosDev->begin() + 1);
}
...
*/
void Humanos::inicializarHumano(int id, int e, int x, int y, int l, int q,
...
*/
void Humanos::inserirHumanos(int n, int estado, int sexo, int fe, int mov,
                             int &i) {
  ...
  for (int j = 0; j < n; ++j) {
    ...
    if (rand() <= PROBABILIDADE_DISTRIBUICAO_HUMANOS(rand())) {
      ...
      if (rand() <= 0.5) {
        ...
        p = ENTRE_FAIXA(ambiente->indPosReg[0], ambiente->indPosReg[1], rand());
      } else {
        // Escolhe uma posição rural.
        p = ENTRE_FAIXA(ambiente->indPosReg[1], ambiente->indPosReg[2], rand());
      }
      x = ambiente->pos[p].x;
      y = ambiente->pos[p].y;
      ...
      t = ENTRE_FAIXA(ambiente->indTrajFE[fe], ambiente->indTrajFE[fe + 1],
                      rand());
      ...
      x = ambiente->pos[ambiente->indPos[ambiente->indQuadras[q * 2] + l] + p]
              .x;
      y = ambiente->pos[ambiente->indPos[ambiente->indQuadras[q * 2] + l] + p]
              .y;
    }
    ...
    for (int idade = BEBE; idade <= IDOSO; ++idade) {
      for (int mov = LOCAL; mov <= TRAJETO; ++mov) {
        for (int estado : {SUSCETIVEL, EXPOSTO, INFECTANTE, RECUPERADO}) {
          inserirHumanos(parametros->parametros[desl], estado, sexo, idade, mov,
                         i);
