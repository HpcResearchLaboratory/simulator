...
*/
ContatoHumanos::ContatoHumanos(Mosquitos *mosquitos, Humanos *humanos,
                               Ambiente *ambiente, Parametros *parametros,
                               int ciclo, int periodo, Seeds *seeds) {
...
*/
__host__ __device__ void ContatoHumanos::operator()(int id) {
  dre &seed = seeds[id];
  ...
  for (int idMosquito = indInicialM; idMosquito < indFinalM; ++idMosquito) {
    ...
    if (ta_m == NENHUM or am_m == ALIMENTADO)
      continue;
    ...
    for (int idHumano = indInicialH; idHumano < indFinalH; ++idHumano) {
      ...
      if (l_h != l or x_h != x or y_h != y or sd_h == MORTO)
        continue;

      // Aplica a redução de atividade noturna
      double chanceRepasto = TAXA_REPASTO;
      if (periodo == NOITE)
        chanceRepasto *= REDUCAO_CONTATO_MH;
...
*/
__host__ __device__ void ContatoHumanos::infeccaoHumano(int idMosquito,
                                                        int idHumano,
                                                        urd<double> dist,
                                                        dre &seed) {
  ...
  int sd_m = GET_SD_M(idMosquito);
  int sd_h = GET_SD_H(idHumano);
  if (sd_m != INFECTANTE or sd_h != SUSCETIVEL)
    return;
  ...
  if (contraiuEsteSorotipo(sc_h, st_m))
    return;
...
/*
  ...
  é iniciado em zero.
*/
__host__ __device__ void ContatoHumanos::infeccaoMosquito(int idMosquito,
                                                          int idHumano,
                                                          urd<double> dist,
                                                          dre &seed) {
  ...
  int sd_m = GET_SD_M(idMosquito);
  int sd_h = GET_SD_H(idHumano);
  if (sd_m != SUSCETIVEL or sd_h != INFECTANTE)
    return;
...
*/
__host__ __device__ bool ContatoHumanos::contraiuEsteSorotipo(int sc_h,
                                                              int st_m) {
