...
#include <simulator/macros/2_CON_H.hpp>
#include <simulator/macros/3_TRA_H.hpp>
#include <simulator/macros/3_TRA_M.hpp>
#include <simulator/macros/4_CON_H.hpp>
#include <simulator/macros/4_CON_M.hpp>
#include <simulator/macros/5_GER_M.hpp>
#include <simulator/macros/general.hpp>
#include <simulator/macros/os.hpp>

#include <simulator/mosquito/contact.hpp>
#include <simulator/mosquito/control.hpp>
#include <simulator/mosquito/generation.hpp>
#include <simulator/mosquito/insertion.hpp>
#include <simulator/mosquito/mosquito.hpp>
#include <simulator/mosquito/movement.hpp>
#include <simulator/mosquito/output.hpp>
#include <simulator/mosquito/transition.hpp>

#include <simulator/human/contact.hpp>
#include <simulator/human/control.hpp>
#include <simulator/human/human.hpp>
#include <simulator/human/insertion.hpp>
#include <simulator/human/movement.hpp>
#include <simulator/human/output.hpp>
#include <simulator/human/transition.hpp>
...
Simulacao::Simulacao(int idSim, string saidaSim, Saidas *saidas,
  ...
  seeds = new Seeds(
      {mosquitos->maxMosquitos, humanos->nHumanos, ambiente->sizePos});
...
/*
  ...
  agentes posicionados na localidade "10". A atualização dos índices é
  necessária pois a movimentação pode alterar a localidade em que os humanos
  estão posicionados.
*/
void Simulacao::movimentacaoHumanos() {
  // auto start = high_resolution_clock::now();
  for_each_n(seeds->idx, humanos->nHumanos,
...
/*
  ...
  os agentes posicionados na localidade "10". A atualização dos índices é
  necessária pois a movimentação pode alterar a localidade em que os mosquitos
  estão posicionados.
*/
void Simulacao::movimentacaoMosquitos() {
  // auto start = high_resolution_clock::now();
  for_each_n(seeds->idx, mosquitos->nMosquitos,
             MovimentacaoMosquitos(mosquitos, humanos, ambiente, parametros,
                                   periodo, ambiente->sizePontEst, seeds));
...
/*
  ...
  sobre os dados, dependendo das flags utilizadas durante a compilação
  realizada.
*/
void Simulacao::contatoEntreMosquitos(int periodo) {
  // auto start = high_resolution_clock::now();
  for_each_n(seeds->idx, ambiente->sizePos,
...
/*
  ...
  ocorrem a transmissão da doença de agentes infectados para agentes
  suscetíveis.
...
void Simulacao::vacinacao() {
  for_each_n(seeds->idx, ambiente->sizeQuadVac,
             CampanhaVacinacao(humanos, ambiente, parametros, ciclo,
                               ambiente->sizeQuadVac, ambiente->sizeFEVac,
                               ambiente->sizePerVac, ambiente->sizeCicVac,
                               seeds));
  for_each_n(seeds->idx, 1,
             PosCampanhaVacinacao(ambiente, ciclo, ambiente->sizePerVac,
                                  ambiente->sizeCicVac));

  for_each_n(seeds->idx, ambiente->sizeVacs,
             InsercaoVacinados(humanos, ambiente, parametros, ciclo, seeds));
}
...
/*
  ...
  O método "for_each_n" é responsável pela aplicação do operador
  "ControleNaturalMosquitosPorIdade" sobre toda a população de agentes
  mosquitos. Como a biblioteca Thrust é utilizada, a aplicação desta operação
  pode ocorrer paralelamente sobre os dados, dependendo das flags utilizadas
  durante a compilação realizada.
*/
...
void Simulacao::controleNaturalMosquitosPorSelecao() {
  // auto start = high_resolution_clock::now();
  for_each_n(seeds->idx, ambiente->sizePos,
             ControleNaturalMosquitosPorSelecao(mosquitos, ambiente, parametros,
                                                ciclo, idLira, seeds));
  // auto stop = high_resolution_clock::now();
  // auto duration = duration_cast<microseconds>(stop - start);
...
void Simulacao::controlesMosquitos() {
  for (int idContr = 0; idContr < ambiente->sizeContr and
                        ambiente->contr[idContr].ciclo <= ciclo;
       ++idContr) {
    for_each_n(
        seeds->idx, 1,
        PreControlesMosquitos(ambiente, parametros, ciclo, idContr, seeds));

    for_each_n(seeds->idx, ambiente->sizePos,
               ControlesMosquitos(mosquitos, ambiente, parametros, ciclo,
                                  idContr, ambiente->sizePontEst, seeds));

    for_each_n(seeds->idx, 1,
               PosControlesMosquitos(ambiente, parametros, idContr, seeds));
  }
}
...
void Simulacao::controleBiologico() {
  int n =
      transform_reduce(seeds->idx, seeds->idx + ambiente->sizeConBio,
                       PreControleBiologico(parametros, ciclo), 0, plus<int>());
  if (n > 0) {
    int m = count_if(mosquitos->mosquitosDev->begin(),
                     mosquitos->mosquitosDev->end(), EstaMortoMosquito());
    ...
    partition(mosquitos->mosquitosDev->begin(), mosquitos->mosquitosDev->end(),
              EstaMortoMosquito());

    for_each_n(seeds->idx, ambiente->sizeConBio,
               ControleBiologico(mosquitos, ambiente, parametros, ciclo,
                                 mosquitos->alocarMosquitos, seeds));
...
void Simulacao::tratamentoAmbiental() {
  for_each_n(seeds->idx, ambiente->sizeConAmb,
             TratamentoAmbiental(mosquitos, ambiente, parametros, ciclo,
                                 ambiente->sizeConAmb, seeds));
}
...
void Simulacao::geracao() {
  // auto start = high_resolution_clock::now();
  int n = transform_reduce(seeds->idx, seeds->idx + mosquitos->nMosquitos,
  ...
  if (n > 0) {
    ...
    for_each_n(seeds->idx, 1, Geracao(ambiente, mosquitos, parametros, seeds));
...
void Simulacao::insercaoMosquitos() {
  // auto start = high_resolution_clock::now();
  int n =
      transform_reduce(seeds->idx, seeds->idx + 1,
...
void Simulacao::insercaoHumanos() {
  // auto start = high_resolution_clock::now();
  int n = transform_reduce(seeds->idx, seeds->idx + 1,
                           PreInsercaoHumanos(parametros, ciclo, ambiente), 0,
                           plus<int>());
  if (n > 0) {
    int m = count_if(humanos->humanosDev->begin(), humanos->humanosDev->end(),
                     EstaMortoHumano());
    ...
    partition(humanos->humanosDev->begin(), humanos->humanosDev->end(),
              EstaMortoHumano());
...
/*
  ...
  cada posição do ambiente. As chamadas aos métodos "for_each_n" são
  responsáveis pela aplicação dos operadores sobre os dados.
*/
void Simulacao::computarSaidas() {
  for_each_n(seeds->idx, N_COLS_H, ContPopTH(humanos, saidas, ciclo - 1));
  for_each_n(seeds->idx, N_COLS_H, ContPopQH(humanos, saidas, ciclo - 1));
  for_each_n(seeds->idx, N_COLS_H, ContPopNovoQH(humanos, saidas, ciclo - 1));
  for_each_n(
      seeds->idx, ambiente->sizePos,
      ContEspacialH(humanos, saidas, ambiente, parametros->nCiclos, ciclo - 1));
  for_each_n(seeds->idx, ambiente->sizePos,
             ContEspacialNovoH(humanos, saidas, ambiente, parametros->nCiclos,
                               ciclo - 1));
  for_each_n(seeds->idx, N_COLS_H, ContPopNovoTH(humanos, saidas, ciclo - 1));
  for_each_n(seeds->idx, N_COLS_H, ContPopNovoQH(humanos, saidas, ciclo - 1));

  for_each_n(seeds->idx, N_COLS_MD, ContPopTMD(mosquitos, saidas, ciclo - 1));
  for_each_n(seeds->idx, N_COLS_MD, ContPopQMD(mosquitos, saidas, ciclo - 1));
  for_each_n(seeds->idx, ambiente->sizePos,
             ContEspacialMD(mosquitos, saidas, ambiente, parametros->nCiclos,
                            ciclo - 1));
  for_each_n(seeds->idx, N_COLS_MD,
             ContPopNovoTMD(mosquitos, saidas, ciclo - 1));

  for_each_n(seeds->idx, N_COLS_MW, ContPopTMW(mosquitos, saidas, ciclo - 1));
  for_each_n(seeds->idx, N_COLS_MW, ContPopQMW(mosquitos, saidas, ciclo - 1));
}
...
/*
  ...
  métodos "getMemoriaGPU" das distintas classes com dados relevantes à
  simulação. Como os métodos retornam a quantidade de mémoria em bytes, este
  valor é convertido para MB para facilitar a leitura. São considerados os dados
  das classes "Seeds", "Humanos", "Mosquitos", "Saidas", "Parametros" e
  "Ambiente".
*/
